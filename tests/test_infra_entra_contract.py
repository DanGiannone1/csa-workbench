...
def test_deployment_guidance_allows_an_explicitly_authorized_cli_agent_without_weakening_confirmation() -> None:
    ...
    assert 'Resource-group-wide allowlisting belongs in Azure Policy or Resource Graph governance' in deployment_text
...
def test_portable_verifier_tolerates_unrelated_resource_group_resources() -> None:
    code, env = _verifier_fixture()
    governed = _with_smart_detection(env)
...
def test_portable_verifier_accepts_complete_fixture_and_rejects_wiring_roles_and_inventory() -> None:
    ...
    cases = [('APPS', lambda value: value.replace('acr.azurecr.io/csa-workbench-frontend:', 'wrong/')), ('COSMOS_DNS_RECORDS', lambda value: value.replace('10.42.0.40','10.42.0.99')), ('COSMOS_DNS_GROUPS', lambda value: value.replace('["10.42.0.40"]', '[{"ipAddress":"10.42.0.40"}]')), ('ASSIGNMENTS', lambda value: value[:-2]+',{"scope":"/subscriptions/sub/resourceGroups/csa-wb-mvp1-rg/providers/Microsoft.Storage/storageAccounts/storage","roleDefinitionName":"Reader","principalId":"api"}]]'), ('COSMOS_SQL_ASSIGNMENTS', lambda value: value[:-1]+',{"roleDefinitionId":"x","scope":"x","principalId":"api"}]')]
