...
OPENAI_PRIVATE_ENDPOINT_NAME="${BASE_NAME}-openai-pe"
...
APP_INSIGHTS_NAME="${BASE_NAME}-insights"
...
COGNITIVE_SERVICES_PRIVATE_DNS_ZONE='privatelink.cognitiveservices.azure.com'
AI_SERVICES_PRIVATE_DNS_ZONE='privatelink.services.ai.azure.com'
...
verify_inventory() {
  local apps deployments identities resources system_topics system_topic_name system_topic_subscriptions acr azure_open_ai foundry_project cosmos storage vnet private_endpoints private_dns_zones managed_environment network_security_groups cosmos_dns_links storage_dns_links openai_dns_links cognitive_services_dns_links ai_services_dns_links cosmos_dns_groups storage_dns_groups openai_dns_groups cosmos_dns_records storage_dns_records openai_dns_records cognitive_services_dns_records ai_services_dns_records frontend_principal api_principal runtime_principal assignments cosmos_sql_assignments app_insights log_analytics
...
PY
  ...
  app_insights="$(az resource show -g "$RESOURCE_GROUP" -n "$APP_INSIGHTS_NAME" --resource-type Microsoft.Insights/components -o json)"
  log_analytics="$(az resource show -g "$RESOURCE_GROUP" -n "$LOG_ANALYTICS_NAME" --resource-type Microsoft.OperationalInsights/workspaces -o json)"
  ...
  openai_dns_links="$(az network private-dns link vnet list -g "$RESOURCE_GROUP" --zone-name "$OPENAI_PRIVATE_DNS_ZONE" -o json)"
  cognitive_services_dns_links="$(az network private-dns link vnet list -g "$RESOURCE_GROUP" --zone-name "$COGNITIVE_SERVICES_PRIVATE_DNS_ZONE" -o json)"
  ai_services_dns_links="$(az network private-dns link vnet list -g "$RESOURCE_GROUP" --zone-name "$AI_SERVICES_PRIVATE_DNS_ZONE" -o json)"
  ...
  openai_dns_groups="$(az network private-endpoint dns-zone-group list -g "$RESOURCE_GROUP" --endpoint-name "$OPENAI_PRIVATE_ENDPOINT_NAME" -o json)"
  ...
  openai_dns_records="$(az network private-dns record-set a list -g "$RESOURCE_GROUP" -z "$OPENAI_PRIVATE_DNS_ZONE" -o json)"
  cognitive_services_dns_records="$(az network private-dns record-set a list -g "$RESOURCE_GROUP" -z "$COGNITIVE_SERVICES_PRIVATE_DNS_ZONE" -o json)"
  ai_services_dns_records="$(az network private-dns record-set a list -g "$RESOURCE_GROUP" -z "$AI_SERVICES_PRIVATE_DNS_ZONE" -o json)"
...
APP_INSIGHTS_CONNECTION_STRING="$(foundation_output appInsightsConnectionString)"
[[ -n "$ENVIRONMENT_DOMAIN" && -n "$ACR_SERVER" && -n "$ACR_NAME" && -n "$AOAI_NAME" && -n "$API_PRINCIPAL_ID" && -n "$APP_INSIGHTS_CONNECTION_STRING" ]] || fail 'foundation deployment did not return required outputs'
