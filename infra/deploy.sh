...
if app_insights.get('name') != os.environ['APP_INSIGHTS_NAME'] or app_insights.get('properties', {}).get('provisioningState') != 'Succeeded' or app_insights.get('properties', {}).get('WorkspaceResourceId', '').lower() != expected_workspace_id or not isinstance(app_insights_connection, str) or not app_insights_connection: raise SystemExit('Application Insights profile drifted')
if log_analytics.get('name') != os.environ['LOG_ANALYTICS_NAME'] or log_analytics.get('properties', {}).get('provisioningState') != 'Succeeded': raise SystemExit('Log Analytics workspace profile drifted')
