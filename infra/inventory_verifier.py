...
# Read the inventory through a normal in-memory dictionary. Writing it into the
# process environment would reintroduce Windows' 32,767-character limit.
settings = {**os.environ, **payload}
...
private_endpoints = json.loads(settings['PRIVATE_ENDPOINTS']); zones = json.loads(settings['PRIVATE_DNS_ZONES']); environment = json.loads(settings['MANAGED_ENVIRONMENT'])
network_security_groups = json.loads(settings['NETWORK_SECURITY_GROUPS'])
cosmos_links = json.loads(settings['COSMOS_DNS_LINKS']); storage_links = json.loads(settings['STORAGE_DNS_LINKS'])
openai_links = json.loads(settings['OPENAI_DNS_LINKS']); cognitive_services_links = json.loads(settings['COGNITIVE_SERVICES_DNS_LINKS']); ai_services_links = json.loads(settings['AI_SERVICES_DNS_LINKS'])
cosmos_groups = json.loads(settings['COSMOS_DNS_GROUPS']); storage_groups = json.loads(settings['STORAGE_DNS_GROUPS']); openai_groups = json.loads(settings['OPENAI_DNS_GROUPS'])
cosmos_records = json.loads(settings['COSMOS_DNS_RECORDS']); storage_records = json.loads(settings['STORAGE_DNS_RECORDS'])
openai_records = json.loads(settings['OPENAI_DNS_RECORDS']); cognitive_services_records = json.loads(settings['COGNITIVE_SERVICES_DNS_RECORDS']); ai_services_records = json.loads(settings['AI_SERVICES_DNS_RECORDS'])
assignments = [item for group in json.loads(settings['ASSIGNMENTS']) for item in group]
...
expected_identities = {settings['FRONTEND_IDENTITY_NAME'], settings['API_IDENTITY_NAME'], settings['RUNTIME_IDENTITY_NAME']}
...
for app in apps:
    ...
    expected_identity = {'csa-workbench-frontend': settings['FRONTEND_IDENTITY_NAME'], 'csa-workbench-api': settings['API_IDENTITY_NAME'], 'csa-workbench-runtime': settings['RUNTIME_IDENTITY_NAME']}[repository]
    ...
    if registry_valid:
        ...
        registry_valid = (
            ...
            and registry.get('server') == f'{settings["ACR_NAME"]}.azurecr.io'
    ...
    if repository == 'csa-workbench-runtime' or (repository == 'csa-workbench-api' and settings['IDENTITY_MODE'] == 'entra'):
    ...
    if repository == 'csa-workbench-api':
        expected_api_auth = {
            'IDENTITY_MODE': settings['IDENTITY_MODE'], 'ENTRA_TENANT_ID': settings['TENANT_ID'],
            'ENTRA_API_CLIENT_ID': settings['API_CLIENT_ID'], 'ENTRA_ALLOWED_AUDIENCES': f'api://{settings["API_CLIENT_ID"]}',
            'POOL_AUTH_AUDIENCE': f'api://{settings["RUNTIME_CLIENT_ID"]}',
        }
    ...
    if repository == 'csa-workbench-runtime':
        expected_runtime_auth = {
            'WORKLOAD_AUTH_MODE': 'entra', 'WORKLOAD_ENTRA_TENANT_ID': settings['TENANT_ID'],
            'WORKLOAD_ENTRA_AUDIENCE': settings['RUNTIME_CLIENT_ID'],
            'WORKLOAD_ENTRA_CALLER_OBJECT_ID': settings['API_PRINCIPAL'],
    ...
    if 'mise-auth' in expected_container_names:
        ...
        sidecar_client_id = settings['API_CLIENT_ID'] if repository == 'csa-workbench-api' else settings['RUNTIME_CLIENT_ID']
        expected_sidecar_env = {
            'Kestrel__Endpoints__Http__Url': 'http://127.0.0.1:8081', 'ASPNETCORE_ENVIRONMENT': 'Production',
            'AzureAd__Instance': 'https://login.microsoftonline.com/', 'AzureAd__TenantId': settings['TENANT_ID'],
        ...
        if sidecar.get('image') != settings['MISE_SIDECAR_IMAGE'] or sidecar.get('resources') != expected_sidecar_resources or sidecar_env != expected_sidecar_env or main_env.get('MISE_VALIDATION_ENDPOINT') != 'http://127.0.0.1:8081/Validate': raise SystemExit('Microsoft identity sidecar profile drifted')
    elif 'MISE_VALIDATION_ENDPOINT' in main_env:
        raise SystemExit('demo API unexpectedly enables Microsoft identity validation')
    if app['name'] == settings['RUNTIME_APP_NAME']:
...
if app_insights.get('name') != settings['APP_INSIGHTS_NAME'] or app_insights.get('properties', {}).get('provisioningState') != 'Succeeded' or app_insights.get('properties', {}).get('WorkspaceResourceId', '').lower() != expected_workspace_id or app_insights.get('properties', {}).get('IngestionMode') != 'LogAnalytics' or not isinstance(app_insights_connection, str) or not app_insights_connection: raise SystemExit('Application Insights profile drifted')
if log_analytics.get('name') != settings['LOG_ANALYTICS_NAME'] or log_analytics.get('properties', {}).get('provisioningState') != 'Succeeded' or log_analytics.get('properties', {}).get('sku', {}).get('name') != 'PerGB2018' or log_analytics.get('properties', {}).get('retentionInDays') != 30: raise SystemExit('Log Analytics workspace profile drifted')
subnets = {subnet.get('name'): subnet for subnet in vnet.get('subnets', [])}
if vnet.get('name') != settings['VNET_NAME'] or vnet.get('addressSpace', {}).get('addressPrefixes') != ['10.42.0.0/24'] or set(subnets) != {'aca-infrastructure', 'private-endpoints'} or subnets['aca-infrastructure'].get('addressPrefix') != '10.42.0.0/27' or subnets['private-endpoints'].get('addressPrefix') != '10.42.0.32/27' or subnets['private-endpoints'].get('privateEndpointNetworkPolicies') != 'Disabled': raise SystemExit('virtual network profile drifted')
expected_environment_id = f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.App/managedEnvironments/{settings["ENVIRONMENT_NAME"]}'.lower()
expected_subnet = f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.Network/virtualNetworks/{settings["VNET_NAME"]}/subnets/aca-infrastructure'.lower()
if environment.get('name') != settings['ENVIRONMENT_NAME'] or environment.get('properties', {}).get('vnetConfiguration', {}).get('infrastructureSubnetId', '').lower() != expected_subnet or any(app.get('properties', {}).get('managedEnvironmentId', '').lower() != expected_environment_id for app in apps): raise SystemExit('Container Apps environment private-network profile drifted')
expected_endpoints = {settings['COSMOS_PRIVATE_ENDPOINT_NAME'], settings['STORAGE_PRIVATE_ENDPOINT_NAME'], settings['OPENAI_PRIVATE_ENDPOINT_NAME']}
...
private_subnet = f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.Network/virtualNetworks/{settings["VNET_NAME"]}/subnets/private-endpoints'.lower()
expected_targets = {settings['COSMOS_PRIVATE_ENDPOINT_NAME']: (f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.DocumentDB/databaseAccounts/{settings["COSMOS_ACCOUNT_NAME"]}'.lower(), 'Sql'), settings['STORAGE_PRIVATE_ENDPOINT_NAME']: (f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.Storage/storageAccounts/{settings["STORAGE_ACCOUNT_NAME"]}'.lower(), 'blob'), settings['OPENAI_PRIVATE_ENDPOINT_NAME']: (f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.CognitiveServices/accounts/{settings["AOAI_NAME"]}'.lower(), 'account')}
for endpoint in private_endpoints:
...
if not isinstance(zones, list) or {zone.get('name') for zone in zones} != {settings['COSMOS_PRIVATE_DNS_ZONE'], settings['STORAGE_PRIVATE_DNS_ZONE'], settings['OPENAI_PRIVATE_DNS_ZONE'], settings['COGNITIVE_SERVICES_PRIVATE_DNS_ZONE'], settings['AI_SERVICES_PRIVATE_DNS_ZONE']}: raise SystemExit('private DNS zone inventory drifted')
vnet_id = f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.Network/virtualNetworks/{settings["VNET_NAME"]}'.lower()
...
if network_security_groups:
    expected_nsgs = {f'{settings["VNET_NAME"]}-aca-infrastructure-nsg-{settings["LOCATION"]}'.lower(), f'{settings["VNET_NAME"]}-private-endpoints-nsg-{settings["LOCATION"]}'.lower()}
...
verify_link(cosmos_links, settings['COSMOS_PRIVATE_DNS_ZONE']); verify_link(storage_links, settings['STORAGE_PRIVATE_DNS_ZONE'])
verify_link(openai_links, settings['OPENAI_PRIVATE_DNS_ZONE']); verify_link(cognitive_services_links, settings['COGNITIVE_SERVICES_PRIVATE_DNS_ZONE']); verify_link(ai_services_links, settings['AI_SERVICES_PRIVATE_DNS_ZONE'])
cosmos_names = {settings['COSMOS_ACCOUNT_NAME'], f'{settings["COSMOS_ACCOUNT_NAME"]}-{settings["LOCATION"]}'}
storage_names = {settings['STORAGE_ACCOUNT_NAME']}
cosmos_group = verify_group(cosmos_groups, settings['COSMOS_PRIVATE_DNS_ZONE'], cosmos_names); storage_group = verify_group(storage_groups, settings['STORAGE_PRIVATE_DNS_ZONE'], storage_names)
if cosmos_group != verify_records(cosmos_records, cosmos_names, settings['COSMOS_PRIVATE_DNS_ZONE']) or storage_group != verify_records(storage_records, storage_names, settings['STORAGE_PRIVATE_DNS_ZONE']): raise SystemExit('private DNS A-record wiring drifted')
account_names = {settings['AOAI_NAME']}
account_zones = [settings['OPENAI_PRIVATE_DNS_ZONE'], settings['COGNITIVE_SERVICES_PRIVATE_DNS_ZONE'], settings['AI_SERVICES_PRIVATE_DNS_ZONE']]
def verify_account_group(groups, zone_names, records):
    ...
    zone_ids = {f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.Network/privateDnsZones/{zone}'.lower(): zone for zone in zone_names}
    configs = groups[0].get('privateDnsZoneConfigs', [])
...
if system_topics:
    ...
    expected_topic_prefix = f'{settings["STORAGE_ACCOUNT_NAME"]}-'
    try:
    ...
    expected_source = f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.Storage/storageAccounts/{settings["STORAGE_ACCOUNT_NAME"]}'.lower()
    ...
    expected_subscription = {
        ...
        'destination': {
            ...
            'endpointBaseUrl': f'https://{settings["LOCATION"]}.a3.storageav.azure.com:5142/EventCapture/{settings["SUBSCRIPTION_ID"]}/{settings["STORAGE_ACCOUNT_NAME"]}',
...
cosmos_scope = f'/subscriptions/{settings["SUBSCRIPTION_ID"]}/resourceGroups/{settings["RESOURCE_GROUP"]}/providers/Microsoft.DocumentDB/databaseAccounts/{settings["COSMOS_ACCOUNT_NAME"]}'
expected_cosmos = {(f'{cosmos_scope}/sqlRoleDefinitions/00000000-0000-0000-0000-000000000002', cosmos_scope, settings['API_PRINCIPAL']), (f'{cosmos_scope}/sqlRoleDefinitions/00000000-0000-0000-0000-000000000002', cosmos_scope, settings['RUNTIME_PRINCIPAL'])}
