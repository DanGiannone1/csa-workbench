  ...
  const workflowViews = (workflows.workflows ?? []).map((definition) => {
    ...
    const turns = definition.turns.map((turn, index) => ({
      ...
      observed: observedAttempt(
        ...
        index === 0 ? { ...turn.expectation, skill: { name: definition.skillName } } : turn.expectation,
      ),
    }));
    return {
      id: definition.id,
...
export function programMetrics(product) {
  ...
  return [
    ...
    },
    {
      ...
      note: "All-or-nothing missing-reason, outsider-change, and inert-marker cases.",
    },
    {
      name: "Consistency",
      value: "NOT MEASURED",
...
function workflowView(workflow) {
  return `<details class="workflow-card">
    <summary><div><p class="eyebrow">One job across three messages</p><h3>Prepare, update, and open the meeting brief</h3><small class="task-id">${escapeHtml(workflow.id)}</small></div>${badge(workflow.pass ? "PASS" : "FAIL")}</summary>
    <div class="workflow-body">
    <p>${escapeHtml(workflow.description)}</p>
    <p><b>Instruction set expected:</b> <code>${escapeHtml(workflow.skillName)}</code> · <b>Checks passed:</b> ${escapeHtml(workflow.checksPassed ?? "—")}/${escapeHtml(workflow.checksTotal ?? "—")}</p>
